...
bot.onText(/\/portfolio/, async (msg) => {
  ...
  bot.sendMessage(chatId, '⏳ 포트폴리오 조회 중...');

  try {
    const portfolioData = await getPortfolio();
    if (!portfolioData) {
      bot.sendMessage(chatId, '❌ 포트폴리오 조회에 실패했습니다.');
      return;
    }
 ...
 * /price 명령
 */
bot.onText(/\/price\s+(\d+)/, async (msg, match) => {
  const chatId = msg.chat.id;
  const stockCode = match[1].padStart(6, '0');

  bot.sendMessage(chatId, `⏳ ${stockCode} 조회 중...`);

  try {
    const stock = await getStockPrice(stockCode);
    if (!stock || stock.error) {
      bot.sendMessage(chatId, `❌ 종목을 찾을 수 없습니다: ${stockCode}`);
      return;
    }
...
bot.onText(/\/refresh/, async (msg) => {
  ...
  bot.sendMessage(chatId, '🔄 포트폴리오 새로고침 중...');

  try {
    const portfolioData = await getPortfolio();
    if (!portfolioData) {
      bot.sendMessage(chatId, '❌ 포트폴리오 새로고침 실패');
      return;
    }

    const text = `✅ 포트폴리오 새로고침 완료!\n\n${formatPortfolioText(portfolioData)}`;
    bot.sendMessage(chatId, text, { parse_mode: 'Markdown' });
  } catch (error) {
    bot.sendMessage(chatId, `❌ 오류: ${error.message}`);
  }
...
`;
    ...
    bot.sendMessage(chatId, `⚠️ 서버 연결 안됨: 백엔드 서버(http://localhost:3000)가 실행 중인지 확인하세요.`);
  }
});
...
📋 *포트폴리오 관련:*
/portfolio - 전체 포트폴리오 현황 (평가액, 손익, 수익률)
